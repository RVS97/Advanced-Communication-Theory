...
for i = 1:size(paths,1)
    for j = 1:paths(i)
        ...
        X(n,:) = beta(n)*X(n,:); % multiply by fading coeff
        n = n + 1;
    end
end

symbolsOut = S*X; % multiply by manifold

% Add Noise
Ps = (symbolsOut*symbolsOut')/length(symbolsOut); % signal power
Pn = Ps/db2mag(SNR); % noise power according to SNR
noise = sqrt(Pn/2)*randn(1,length(symbolsOut)) + 1i*sqrt(Pn/2)*randn(1,length(symbolsOut)); % noise
